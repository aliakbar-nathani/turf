...
class OwnerTurfEditScreen extends StatefulWidget {
  final Turf turf;
  
  const OwnerTurfEditScreen({super.key, required this.turf});

  @override
  State<OwnerTurfEditScreen> createState() => _OwnerTurfEditScreenState();
}

class _OwnerTurfEditScreenState extends State<OwnerTurfEditScreen> {
  ...
  void initState() {
    ...
    _descriptionController = TextEditingController(text: widget.turf.description);
    _addressController = TextEditingController(text: widget.turf.address);
    _cityController = TextEditingController(text: widget.turf.city);
    _stateController = TextEditingController(text: widget.turf.state);
    _countryController = TextEditingController(text: widget.turf.country);
    _postalCodeController = TextEditingController(text: widget.turf.postalCode);
    _priceController = TextEditingController(text: widget.turf.basePricePerHour.toString());
    _featuresController = TextEditingController(text: widget.turf.features.join(', '));
    _sizeController = TextEditingController(text: widget.turf.size);
    _imageUrlController = TextEditingController(text: widget.turf.imageUrl);
    _additionalImagesController = TextEditingController(
      text: widget.turf.additionalImages.join(', ')
    );
    
    // Initialize form values
    _indoor = widget.turf.indoor;
    _hasParking = widget.turf.hasParking;
    _hasChangingRoom = widget.turf.hasChangingRoom;
    _hasShower = widget.turf.hasShower;
    _hasFloodlights = widget.turf.hasFloodlights;
    _hasEquipment = widget.turf.hasEquipment;
    _surfaceType = widget.turf.surfaceType;
  ...
  Future<void> _saveTurf() async {
    ...
    try {
      ...
      // Update the turf
      final result = await _turfService.updateTurf(widget.turf.id, turfData);
      ...
      if (result['success']) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? 'Turf updated successfully'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? 'Failed to update turf'),
            backgroundColor: Colors.red,
          ),
        );
      }
